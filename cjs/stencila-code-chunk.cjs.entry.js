...
const CodeChunkComponent = class {
  constructor(hostRef) {
    ...
    this.updateErrors = (errors = []) => {
      this.codeErrors = errors.map((error) => (index.h("stencila-code-error", { kind: error.errorType, hasStacktrace: error.stackTrace !== undefined }, error.errorMessage, index.h("pre", { slot: "stacktrace" }, error.stackTrace))));
    };
  ...
  async getContents() {
    ...
    if (this.editorRef) {
      ...
      return schema.l({ text, programmingLanguage: language });
    }
    return schema.l({ text: '' });
  }
  /**
   * Run the `CodeChunk`
   */
  ...
  render() {
    return (index.h(index.Host, { class: {
      ...
      } }, index.h("stencila-action-menu", { expandable: true }, this.executeHandler !== undefined && (index.h("stencila-button", { icon: "play", minimal: true, color: "key", class: "run", size: "xsmall", tooltip: "Run", iconOnly: true, slot: "persistentActions", onClick: this.executeRef, isLoading: this.executeCodeState === 'PENDING' })), index.h("stencila-button", { minimal: true, color: "key", class: "sourceToggle", onClick: this.toggleCodeVisibility, icon: this.isCodeVisibleState ? 'eye-off' : 'eye', iconOnly: true, size: "xsmall", slot: "persistentActions", tooltip: `${this.isCodeVisibleState ? 'Hide' : 'Show'} Code` })), index.h("div", null, index.h("div", { class: {
        editorContainer: true,
        hidden: !this.isCodeVisibleState,
      } }, index.h("stencila-editor", { activeLanguage: this.programmingLanguageProp, autofocus: this.autofocus, executeHandler: this.onExecuteHandler, keymap: this.keymap, readOnly: this.executeHandler === undefined }, index.h("slot", { name: CodeChunkComponent.slots.text }))), index.h("stencila-node-list", { nodes: this.outputs }, index.h("slot", { name: CodeChunkComponent.slots.outputs }), this.codeErrors))));
  }
