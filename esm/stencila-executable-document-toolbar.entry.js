import { h, r as registerInstance, H as Host } from './index-9edcbeb0.js';
import { c as createCommonjsModule$1, a as commonjsGlobal, b as commonjsRequire } from './_commonjsHelpers-8a9f3b18.js';
import { T as ToastTypes, a as toastController, b as ToastPositions } from './toastController-1c8ec0b3.js';
import { u as ut, s as s$1 } from './schema-ceac4a12.js';
 ...
 * @since 2.0.0
 */
var map$1 = function (f) { return function (fa) { return map_(fa, f); }; };

/**
 * @since 2.0.0
 */
const URI = '@nll/datum/Datum';
/**
 ...
 * @since 2.0.0
 */
const constInitial = constant(initial);
/**
 * @since 2.0.0
 */
 ...
 * @since 2.0.0
 */
const isValued = (ma) => isReplete(ma) || isRefresh(ma);
/**
 * @since 2.0.0
 */
const getOrElse = (onInitial, onPending) => (ma) => fold$1(onInitial, onPending, identity, identity)(ma);
/**
 * @since 2.0.0
 */
const mapC = (fa, f) => fold$1(constInitial, constPending, a => refresh(f(a)), a => replete(f(a)))(fa);
/**
 * @since 2.0.0
 */
const apC = (fab, fa) => fold$1(constInitial, () => fold$1(constInitial, constPending, constPending, constPending)(fa), f => fold$1(constInitial, constPending, a => refresh(f(a)), a => refresh(f(a)))(fa), f => fold$1(constInitial, constPending, a => refresh(f(a)), a => replete(f(a)))(fa))(fab);
/**
 * @since 2.0.0
 */
const chainC = (fa, f) => fold$1(constInitial, constPending, f, f)(fa);
/**
 * @since 2.0.0
 */
const reduceC = (fa, b, f) => fold$1(() => b, () => b, a => f(b, a), a => f(b, a))(fa);
/**
 * @since 2.0.0
 */
const foldMapC = (M) => (fa, f) => fold$1(() => M.empty, () => M.empty, f, f)(fa);
/**
 * @since 2.0.0
 */
const reduceRightC = (fa, b, f) => fold$1(() => b, () => b, a => f(a, b), a => f(a, b))(fa);
/**
 * @since 2.0.0
 */
const traverseC = (F) => (ta, f) => fold$1(() => F.of(initial), () => F.of(pending), a => F.map(f(a), refresh), a => F.map(f(a), replete))(ta);
/**
 * @since 2.0.0
 */
const sequenceC = (F) => (ta) => fold$1(() => F.of(initial), () => F.of(pending), a => F.map(a, refresh), a => F.map(a, replete))(ta);
/**
 * @since 2.0.0
 */
 ...
 * @since 2.0.0
 */
const compactC = (fa) => fold$1(constInitial, constPending, a => (isSome(a) ? refresh(a.value) : initial), a => (isSome(a) ? replete(a.value) : initial))(fa);
 ...
 * @since 2.0.0
 */
const filterC = (fa, predicate) => fold$1(constInitial, constPending, a => (predicate(a) ? fa : initial), a => (predicate(a) ? fa : initial))(fa);
/**
 * @since 2.0.0
 */
 ...
 * @since 2.0.0
 */
const witherC = (F) => (fa, f) => fold$1(constant(F.of(initial)), constant(F.of(pending)), a => F.map(f(a), o => (isSome(o) ? refresh(o.value) : initial)), a => F.map(f(a), o => (isSome(o) ? replete(o.value) : initial)))(fa);
/**
 * @since 2.0.0
 */
const wiltC = (F) => (fa, f) => {
...
function getEitherM(M) {
    var ap$1 = function (fga) { return function (fgab) {
        return M.ap(M.map(fgab, function (h) { return function (ga) { return pipe(h, ap(ga)); }; }), fga);
    }; };
    var of = flow(right, M.of);
    return {
        map: function (fa, f) { return M.map(fa, map(f)); },
        ap: function (fab, fa) { return pipe(fab, ap$1(fa)); },
        of: of,
        chain: function (ma, f) { return M.chain(ma, function (e) { return (isLeft(e) ? M.of(left(e.left)) : f(e.right)); }); },
        alt: function (fa, that) { return M.chain(fa, function (e) { return (isLeft(e) ? that() : of(e.right)); }); },
        bimap: function (ma, f, g) { return M.map(ma, function (e) { return pipe(e, bimap(f, g)); }); },
        mapLeft: function (ma, f) { return M.map(ma, function (e) { return pipe(e, mapLeft(f)); }); },
        fold: function (ma, onLeft, onRight) { return M.chain(ma, fold(onLeft, onRight)); },
        getOrElse: function (ma, onLeft) { return M.chain(ma, fold(onLeft, M.of)); },
        orElse: function (ma, f) {
            return M.chain(ma, fold(f, function (a) { return of(a); }));
        },
        swap: function (ma) { return M.map(ma, swap); },
        rightM: function (ma) { return M.map(ma, right); },
        leftM: function (ml) { return M.map(ml, left); },
 ...
 * @since 2.0.0
 */
const URI$1 = '@nll/datum/DatumEither';
/**
 * @since 2.0.0
 */
const datumEither = Object.assign(Object.assign({}, getEitherM(datum)), { URI: URI$1 });
/**
 * @since 2.1.0
 */
const success = (a) => replete(right(a));
/**
 * @since 2.1.0
 */
const failure = (e) => replete(left(e));
/**
 * @since 2.4.1
 */
 ...
 * @since 2.4.1
 */
const constInitial$1 = () => initial$1;
/**
 * @since 2.4.1
 */
const constPending$1 = () => pending$1;
/**
 * @since 2.1.0
 */
 ...
 * @since 2.1.0
 */
const toRefresh = (fea) => fold$1(constPending$1, constPending$1, constant(fea), refresh)(fea);
/**
 * @since 2.7.0
 */
const toReplete = (fea) => fold$1(constInitial$1, constInitial$1, replete, constant(fea))(fea);
/**
 * @since 2.7.0
 */
const fold$2 = (onInitial, onPending, onRefreshLeft, onRefreshRight, onRepleteLeft, onRepleteRight) => (fea) => {
    switch (fea._tag) {
        case 'Initial':
            return onInitial();
        case 'Pending':
            return onPending();
        case 'Refresh':
            return fold(onRefreshLeft, onRefreshRight)(fea.value);
        case 'Replete':
            return fold(onRepleteLeft, onRepleteRight)(fea.value);
    }
};
...
function e(){return (e=Object.assign||function(e){for(var n=1;n<arguments.length;n++){var r=arguments[n];for(var t in r)Object.prototype.hasOwnProperty.call(r,t)&&(e[t]=r[t]);}return e}).apply(this,arguments)}var n,r;if("undefined"!=typeof process&&(n={emit:process.emit,listeners:process.listeners,addListener:process.addListener,removeListener:process.removeListener,removeAllListeners:process.removeAllListeners}),"undefined"!=typeof window){var t=new Map;n={emit:function(e,n){window.dispatchEvent(new CustomEvent(e,{detail:n}));},listeners:function(){return Array.from(t.keys())},addListener:function(e,n){var r=function(e){return n(e.detail)};window.addEventListener(e,r),t.set(n,r);},removeListener:function(e,n){var r=t.get(n);void 0!==r&&(window.removeEventListener(e,r),t.delete(n));},removeAllListeners:function(e){Array.from(t.values()).map(function(n){window.removeEventListener(e,n);}),t.clear();}};}function i(e,t,i){var o="";"object"==typeof e&&void 0!==e.message?o=e.message:"string"==typeof e&&(o=e);var s={tag:t,level:i,message:o};if("object"==typeof e&&void 0!==e.stack)s.stack=e.stack;else if(i<=r.error){var a=new Error;if(void 0!==a.stack){var v=a.stack.split("\n");s.stack=[v[0]].concat(v.slice(3)).join("\n");}}n.emit("stencila:logga",s);}function o(){return n.listeners("stencila:logga")}function s(e,r){void 0===r&&(r={});var t=e,i=r.tags,o=r.maxLevel,s=r.messageRegex,a=r.func;return void 0===i&&void 0===o&&void 0===s&&void 0===a||(t=function(n){(void 0===i||i.includes(n.tag))&&(void 0!==o&&n.level>o||(void 0===s||s.test(n.message))&&(void 0===a||a(n))&&e(n));}),n.addListener("stencila:logga",t),t}!function(e){e[e.error=0]="error",e[e.warn=1]="warn",e[e.info=2]="info",e[e.debug=3]="debug";}(r||(r={}));var d=new Map;function f(n,t){void 0===t&&(t={});var i=n.tag,o=n.level,s=n.message,a=n.stack,v=t.maxLevel;if(!(o>(void 0===v?r.info:v))){var c=t.throttle;if(void 0!==c){var f=(void 0!==c.signature?c.signature:"").replace(/\${tag}/,i).replace(/\${level}/,o.toString()).replace(/\${message}/,s),l=d.get(f);if(void 0!==l){var u=void 0!==c.duration?c.duration:1e3;if(Date.now()-l<u)return}d.set(f,Date.now());}var g="";if("undefined"!=typeof process&&void 0!==process.stderr&&!0!==process.stderr.isTTY)g=JSON.stringify(e({time:(new Date).toISOString()},n));else {var m=o<0?0:o>3?3:o,p=r[m].toUpperCase().padEnd(5," ");g="undefined"!=typeof window?p+" "+i+" "+s:["🚨","⚠","🛈","🐛"][m]+" "+["[31;1m","[33;1m","[34;1m","[30;1m"][m]+p+"[0m [36m"+i+"[0m "+s;var w=t.showStack;void 0!==w&&w&&void 0!==a&&(g+="\n  "+a);}console.error(g);}}function l(e){return {error:function(n){i(n,e,r.error);},warn:function(n){i(n,e,r.warn);},info:function(n){i(n,e,r.info);},debug:function(n){i(n,e,r.debug);}}}0===o().length&&s(f);

var Ft=function(t){return function(e){return Object.keys(t).includes(e)}},Dt=function(t){return function(e){return !!Wt(e)&&Ft(t)(e.type)}},Gt=function(t){var e,r=((e={})[t]=t,e);return Dt(r)},Wt=function(t){return null!=t&&Object.prototype.hasOwnProperty.call(t,"type")},Kt=Gt("Article"),Ut=Gt("Paragraph"),Xt=Gt("ListItem"),Yt=function(t){return null===t?"Null":"boolean"==typeof t?"Boolean":"number"==typeof t?"Number":"string"==typeof t?"Text":Array.isArray(t)?"Array":Wt(t)?t.type:"Object"};
...
const log$3 = l('executa:ws:client');
...
class WebSocketClient extends Client {
    ...
    start() {
        ...
        const socket = (this.socket = new browser(address.url(), generateProtocol(id, address.jwt)));
        socket.onmessage = (event) => this.receive(event.data.toString());
        socket.onclose = (event) => {
            ...
            if (code === 4001) {
                if (logging)
                    log$3.error(`Failed to authenticate with server: ${reason}`);
                return;
            }
            if (retries > 0) {
                if (logging)
...
const jobStatusString = (jobObject) => _function.pipe(jobObject, fold$2(() => 'Requesting session', () => 'Requesting session', jobErrorMessage, (job) => job.statusMessage, jobErrorMessage, (job) => job.statusMessage));
const SessionStatus = ({ session, job }, children) => (h("span", { class: {
  ...
  } }, _function.pipe(session, fold$2(() => '', () => (h("span", null,
  h("stencila-icon", { icon: "loader-2" }),
  h("span", null, jobStatusString(job)))), (refreshingErroredJob) => (h("span", null,
  h("stencila-icon", { icon: "loader-2" }),
  h("span", null, jobErrorMessage(refreshingErroredJob)))), (refreshingSession) => (h("span", null, isEmpty$1(children) ? ([
  h("stencila-icon", { icon: "loader-2" }),
  ...
  isEmpty$1(children) ? null : h("span", null, children)))))));
...
const StencilaExecutableDocumentToolbar = class {
  constructor(hostRef) {
    ...
    this.handleJobStatus = (job, onJobReady, onJobError) => _function.pipe(job, fold$2(() => {
    ...
    this.findSession = async () => {
      ...
      if (isInitial(this.session) || isFailure(this.session)) {
        ...
        return jobFetch
          ...
          .then(() => new Promise((resolve, reject) => this.handleJobStatus(initial$1, resolve, reject)))
          .catch((err) => {
          errorNotification();
    ...
    this.goToActiveNode = () => {
      var _a;
      _function.pipe(this.codeNodeSelector(), lookup$1((_a = this.activeNodeIndex) !== null && _a !== void 0 ? _a : -1), map$1((el) => {
        var _a;
