...
const DataTable = class {
  ...
  render() {
    ...
    return (h("table", { itemtype: "https://schema.stenci.la/Datatable" }, h("thead", null, h("tr", null, cols.map(col => (h("th", null, col.name))))), h("tbody", null, rows.map((_, row) => (h("tr", null, cols.map(col => (h("td", null, col.values[row])))))))));
  }
};
DataTable.style = {
...
export { DataTable as stencila_data_table, ImageObjectComponent as stencila_image_object };
